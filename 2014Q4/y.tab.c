...
{
  "$end", "error", "$undefined", "BinaryForm", "$accept", "program", YY_NULLPTR
};
#endif
...
yyreduce:
  ...
  switch (yyn)
    {
        case 2:
#line 17 "yacc.y" /* yacc.c:1661  */
    {
    char* binary = malloc(strlen((yyvsp[0].binary))*sizeof(char));
    for(int i = 0; i<strlen((yyvsp[0].binary)); i++){
        if((yyvsp[0].binary)[i] == 'I')
          binary[i] = '1';
        else binary[i] = '0';
    }
    int decimal = 0;
    int two = 1;
    for(int i =strlen((yyvsp[0].binary)) -1; i >=0; i--){
        if(binary[i] == '1') 
            decimal += two;
        two *= 2;
    }
...
#line 1232 "y.tab.c" /* yacc.c:1661  */
    break;


#line 1236 "y.tab.c" /* yacc.c:1661  */
      default: break;
    }
